...
public:
  ...
  bool requiresLegalization() const {
    if (!needsLegalization)
      return false;

    // The mapper currently raises needsLegalization for four reasons: special
    // builtins, image-format propagation, multidimensional resource arrays,
    // and resource aliasing. Native untyped raw-buffer aliases are the one
    // alias form that must not enter the legacy typed-pointer legalizer.
    for (const auto builtin : {spv::BuiltIn::HelperInvocation,
                               spv::BuiltIn::SubgroupSize,
                               spv::BuiltIn::SubgroupLocalInvocationId}) {
      if (builtinToVarMap.find(static_cast<uint32_t>(builtin)) !=
          builtinToVarMap.end())
        return true;
    }

    for (const auto &resource : resourceVars) {
      const auto *varDecl = dyn_cast_or_null<VarDecl>(resource.getDeclaration());
      if (!varDecl)
        continue;

      const QualType type = varDecl->getType();
      if (varDecl->getAttr<VKImageFormatAttr>())
        return true;

      if (const auto *arrayType = astContext.getAsConstantArrayType(type)) {
        QualType elemType = arrayType->getElementType();
        if (astContext.getAsConstantArrayType(elemType)) {
    ...
    const bool nativeUntypedRawAliases =
        !spirvOptions.allowedExtensions.empty() &&
        featureManager.isExtensionEnabled(Extension::KHR_untyped_pointers);
    bool sawNativeUntypedRawAlias = false;

    for (const auto &entry : astDecls) {
      SpirvInstruction *instr = entry.second.instr;
      if (!instr || !instr->containsAliasComponent())
        continue;

      const QualType type = entry.first->getType();
      if (nativeUntypedRawAliases &&
          (isByteAddressBuffer(type) || isRWByteAddressBuffer(type))) {
        sawNativeUntypedRawAlias = true;
        continue;
      }
      return true;
    }

    for (const auto &entry : astFunctionDecls) {
      SpirvFunction *fn = entry.second;
      if (!fn || !fn->constainsAliasComponent())
        continue;

      const auto *fnDecl = dyn_cast<FunctionDecl>(entry.first);
      if (!fnDecl)
        return true;
      const QualType type = fnDecl->getReturnType();
      if (nativeUntypedRawAliases &&
          (isByteAddressBuffer(type) || isRWByteAddressBuffer(type))) {
        sawNativeUntypedRawAlias = true;
        continue;
    ...
    // TODO: Replace the legacy boolean with legalization reason bits. This
    // mirrors every current mapper producer so native raw aliases do not
    // suppress unrelated legalization requirements.
    return !sawNativeUntypedRawAlias;
  }
